...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      ...
      if (file === null) {
        return res.status(404).json({ error: "数据文件尚未初始化，请管理员先发布一次项目数据" });
      }
    ...
    if (req.method === "POST") {
      ...
      await updateGitHubFile(FILE_PATH, projects, sha,
        `update projects.json via admin (${new Date().toISOString()})`);
