...
const MuseumPage: React.FC = () => {
  ...
  const collections = useMemo(
    () => memories.filter((m) => m.category === "collection"),
    []
  );
  ...
  return (
    <div className="museum-page">
      ...
      <section className="museum-hall">
        ...
        {/* 横向时间轴 */}
        <div className="museum-timeline-h">
          <div className="museum-timeline-h-track">
            {collections.map((m, i) => (
              <motion.div
                key={m.id}
                className="museum-card-collection"
                ...
                onClick={() => setActive(m)}
              >
                <div className="museum-card-year-tag">{m.year}</div>
                <FilmImage src={m.imageUrl} alt={m.title} />
                <div className="museum-card-info">
                  <h3 className="museum-card-title">{m.title}</h3>
                  <p className="museum-card-desc">{m.description}</p>
                </div>
                <span className="museum-card-zoom-hint">点击放大</span>
              </motion.div>
            ))}
          </div>
        </div>
