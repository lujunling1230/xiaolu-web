...
const KNOWLEDGE = {
  ...
  projects: [
    ...
    {
      name: "伴龄",
      tagline: "AI 养老规划伴侣，从对话到报告，把焦虑变成行动",
      pain_point: "养老规划信息分散，面对养老话题焦虑，退休金缺口不明确，缺乏温暖引导",
      solution: "5大模块：首页（动态问候+天气出行小贴士+快捷入口）、AI规划（对话式渐进采集年龄/担忧/财务→AI生成规划报告）、报告（核心洞察+关键指标+行动建议+寄语+行动采纳追踪）、工具（养老计算器：退休金缺口/储蓄目标/投资增长可视化）、我的（资料管理+历史报告+设置）",
      decision: "对话式渐进采集而非表单填写，降低养老话题的心理门槛；AI同时扮演理财顾问和心理咨询师角色，温暖但不焦虑",
      highlight: "体现「AI对话设计 + 渐进式信息采集 + 报告生成 + 养老金融计算 + 情感化产品思维」能力",
    },
  ],
  methodology: {
...
function getLocalReply(q) {
  ...
  if (lower.includes("项目") || lower.includes("作品") || lower.includes("拿手"))
    return `俊玲有 6 个作品：森林疗愈室、爱情公寓、通关清单、物资管家、漫游指南、回血清单。每个都围绕「用技术传递温度」的理念，覆盖心理健康、游戏化效率、旅行记录等方向 🌿`;
  if (lower.includes("几个") || lower.includes("多少") || lower.includes("作品数"))
    return "俊玲目前有 6 个作品：森林疗愈室、爱情公寓、通关清单、物资管家、漫游指南、回血清单";
