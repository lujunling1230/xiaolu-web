...
const ShareCard: React.FC<{ totalCount: number; streak: number; onClose: () => void }> = ({ totalCount, streak, onClose }) => {
  const buildDataJson = useCallback(() => {
    ...
    const nodeMap = new Map(TAGGED_NODES.map(n => [n.id, n]));
    const recentItems = recent.map(r => {
      const node = nodeMap.get(r.id);
      const rawText = node?.text || r.keywords?.[0] || "完成了一件回血小事";
      const text = rawText.length > 30 ? rawText.slice(0, 30) + "..." : rawText;
      const icon = node?.icon || ICON_POOL[r.id % ICON_POOL.length] || "🌱";
      const d = new Date(r.timestamp);
      const date = `${d.getMonth() + 1}/${d.getDate()}`;
      return { icon, text, date };
    });
...
const RechargePage: React.FC = () => {
  ...
  useEffect(() => { document.title = "回血清单"; track("tool_enter", { tool_name: "回血清单" }); }, []);
